...
def extract_structured_data(file_path: str, model: BaseModel, num_responses: int = 1):
    ...
    Always extract the figures for the year of the report, rather than prior years which may also be present.
    """
...
async def process_pdf(semaphore: asyncio.Semaphore, pdf_filename: str, pdf_path: str):
    """
    Processes a single PDF file by extracting structured data and saving the outputs.
    The semaphore ensures that only a limited number of PDFs are processed concurrently.
    """
    async with semaphore:
        logger.info(f"Processing {pdf_filename}...")
        try:
            # Offload the blocking extraction to a thread.
...
async def process_financial_reports_concurrent():
    """
    Reads the Excel file containing PDF filenames, and processes PDFs concurrently.
    At most 5 PDFs are processed at the same time.
    """
    ...
    semaphore = asyncio.Semaphore(10)  # Limit concurrency to 5 PDFs at a time
    tasks = []
    for index, row in df.iterrows():
        ...
        if os.path.exists(output_file_path_first):
            ...
            continue
        tasks.append(process_pdf(semaphore, pdf_filename, pdf_path))
...
if __name__ == "__main__":
    asyncio.run(process_financial_reports_concurrent())
